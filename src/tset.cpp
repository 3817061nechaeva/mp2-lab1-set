...
istream &operator>>(istream &istr, TSet &s) // ввод
{
  ...
  {
    if (i > -1 && i < s.MaxPower)
      s.BitField.SetBit(s.BitField.GetLength() - 1 - i);
...
{
  TSet A(s.BitField.GetLength());
  for (int i = 0; i < A.BitField.GetLength(); i++)
    if (s.BitField.GetBit(i) != 0)
    {
      A.BitField.SetBit(A.BitField.GetLength() - 1 - i);
    }
  ostr << "{";
  for (int i = 0; i < A.BitField.GetLength(); i++)
  {
    if (A.BitField.GetBit(i) != 0)
      ostr << i << " ";
  }
  ostr << "}";
  return ostr;
